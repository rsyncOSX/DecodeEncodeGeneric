...
public final class EncodeGeneric {
    ...
    /// - Returns: JSON data
    /// - Throws: EncodeError if encoding fails
    public func encodeArray<T: Encodable>(_ array: [T]) throws -> Data {
        do {
    ...
    public func encodeToString<T: Encodable>(_ object: T) throws -> String {
    ...
    /// - Returns: JSON string
    /// - Throws: EncodeError if encoding or string conversion fails
    public func encodeArrayToString<T: Encodable>(_ array: [T]) throws -> String {
    ...
    public func encode<T: Encodable>(_ object: T, toFile filePath: String, atomically: Bool = true) throws {
    ...
    ///   - atomically: If true, writes to a temporary file first (default: true)
    /// - Throws: EncodeError if encoding or file writing fails
    public func encodeArray<T: Encodable>(_ array: [T], toFile filePath: String, atomically: Bool = true) throws {
        guard !filePath.isEmpty else {
            throw EncodeError.invalidFilePath
        }
...
public extension EncodeGeneric {
    ...
    /// - Throws: EncodeError if encoding or string conversion fails
    func encodeToPrettyString<T: Encodable>(_ object: T) throws -> String {
...
public extension EncodeGeneric {
    ...
    func encodeResult<T: Encodable>(_ object: T) -> Result<Data, EncodeError> {
        do {
            let data = try encode(object)
            return .success(data)
    ...
    func encodeToStringResult<T: Encodable>(_ object: T) -> Result<String, EncodeError> {
        do {
